...
4. **Splash Screen Premium com Delay de 2s**: Atualização visual da tela de carregamento com a fonte Outfit (Google Fonts), gradiente radial escuro e barra de carregamento horizontal dourada com efeito shimmer. A logo quadrada (`logo.png`) foi emoldurada em formato squircle com escala de 5% (`scale(1.05)`) e `overflow: hidden` no [index.html](file:///c:/Users/Ellen%20Lisboa/Videos/Barbearia/index.html) para ocultar as arestas brancas do PNG original. O contêiner foi movido para fora da div `#root` e é removido programmaticamente por um timer assíncrono de 2 segundos com transição suave de esmaecimento (`opacity`) e zoom-out (`scale`) em [main.tsx](file:///c:/Users/Ellen%20Lisboa/Videos/Barbearia/src/main.tsx).
5. **Criptografia de Senhas (Hashing SHA-256)**: Implementação de hash criptográfico assíncrono (Web Crypto API) para as credenciais dos barbeiros. Foi desenvolvida uma lógica de migração transparente durante o login (se o login é feito com senha antiga em texto claro, ela é hasheada e atualizada no banco de dados automaticamente). O painel administrativo foi configurado para ocultar as senhas no modo de visualização e edição (usando campos em branco que só atualizam se modificados).
6. **Persistência de Sessão e Autenticação Segura**: Implementação do listener `onAuthStateChanged` do Firebase no carregamento do app, restaurando a sessão administrativa do administrador de forma confiável e eliminando dependências exclusivas de `localStorage` que poderiam sofrer bypass no client-side.
7. **Modularização do Painel Admin**: Divisão do arquivo monolítico [AdminPanel.tsx](file:///c:/Users/Ellen%20Lisboa/Videos/Barbearia/src/components/AdminPanel.tsx) em 4 subcomponentes especializados na pasta `src/components/admin/`: `StatsTab`, `ServicesTab`, `BarbersTab` e `HoursTab`. A alteração reduziu o tamanho e a complexidade do painel em 90%.
8. **Prevenção de Duplicidade de Agendamentos (Double Booking)**: Aprimoramento da gravação de agendamento no Firestore para fazer uma consulta em tempo real (check online) segundos antes da persistência, e alteração do formulário do cliente para aguardar o sucesso da transação antes de limpar os campos de entrada, evitando perda de dados em caso de falha.
